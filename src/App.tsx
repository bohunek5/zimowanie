...
export default function App(_props: Readonly<AppProps>) {
  ...
  return <BrowserRouter basename={import.meta.env.BASE_URL}><Routes><Route element={<Layout theme={theme} onToggleTheme={toggleTheme}/>}><Route index element={<HomePage/>}/><Route path="oferta" element={<OfferPage/>}/><Route path="cennik" element={<PricingPage/>}/><Route path="plac" element={<SitePage/>}/><Route path="hala" element={<HallPage/>}/><Route path="galeria" element={<GalleryPage/>}/><Route path="kontakt" element={<ContactPage/>}/><Route path="*" element={<Navigate to="/" replace/>}/></Route></Routes></BrowserRouter>;
}
