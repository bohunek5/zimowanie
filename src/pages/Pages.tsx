...
export function ContactPage(_props: Readonly<EmptyPageProps>) {
  return <><PageHero label="Kontakt" title="Ustalmy miejsce dla Twojej łodzi." text="Zadzwoń i podaj długość jednostki oraz wybraną formę postoju." image="/assets/gallery/wodowanie-dzwigiem.jpg" imageAlt="Łódź podnoszona dźwigiem nad lawetą"/><section className="section contact-page"><div className="container contact-layout"><a className="contact-main" href={phone.href}><small>Telefon</small><strong>{phone.display}</strong><span>Zadzwoń teraz <ArrowRight/></span></a><div className="contact-info"><article><MapPin/><small>Lokalizacja</small><strong>{address}</strong><p>Wjazd od ul. Sybiraków.</p></article><article><Clock3/><small>Sezon</small><strong>{season}</strong><p>Postój od października do końca kwietnia.</p></article><article><Wrench/><small>Serwis</small><strong>Do ustalenia</strong><p>Przygotowanie możesz zlecić nam albo wykonać samodzielnie.</p></article></div></div></section></>;
}
